...
export function MyTasks() {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold text-[#121212] dark:text-white">My Tasks</h2>
          ...
          <button
            ...
            className={`p-3 rounded-lg border text-center transition-colors ${
              statusFilter === status
                ...
                : 'border-[#D5D5D5] dark:border-[#3A3A3A] hover:bg-[#F5F5F5] dark:hover:bg-[#3A3A3A]/50'
            }`}
          >
            <p className={`text-2xl font-bold ${TASK_STATUS_COLORS[status].text}`}>{counts[status]}</p>
            <p className="text-xs text-[#757575] dark:text-[#9E9E9E] mt-1">{TASK_STATUS_LABELS[status]}</p>
          </button>
        ))}
      </div>
          ...
          {filteredTasks.map((task) => {
            ...
            return (
              <button
                ...
                className="w-full text-left bg-white dark:bg-[#1E1E1E] rounded-xl shadow-sm border border-[#D5D5D5] dark:border-[#3A3A3A] p-4 hover:bg-[#F5F5F5] dark:hover:bg-[#3A3A3A]/50 transition-colors"
              >
                <div className="flex items-start justify-between">
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className="font-medium text-[#121212] dark:text-white truncate">{task.title}</h3>
                      {isOverdue && (
                    ...
                    <p className="text-sm text-[#757575] dark:text-[#9E9E9E] truncate">{task.description}</p>
                  </div>
