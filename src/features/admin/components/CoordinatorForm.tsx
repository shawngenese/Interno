...
export function CoordinatorForm({ editingId, editingCoordinatorId, onCancel, onSaved }: CoordinatorFormProps) {
  ...
  useEffect(() => {
    ...
    init();
  }, [isEditing, editingCoordinatorId, editingId, setFormData]);
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      {!isEditing && (
        <div className="flex items-center gap-2 p-3 bg-muted/30 rounded-lg border border-border">
          <input
            type="checkbox"
            id="useExistingUserCoord"
            checked={useExistingUser}
            onChange={(e) => setUseExistingUser(e.target.checked)}
            className="h-4 w-4 text-primary focus:ring-ring border-input rounded"
          />
          <label htmlFor="useExistingUserCoord" className="text-sm font-medium text-foreground cursor-pointer">
